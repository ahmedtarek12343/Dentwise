import { useGetAvailableDoctors } from "@/hooks/use-doctors";
import {
  Card,
  CardContent,
  CardDescription,
...
function DoctorSelectionStep({
  ...
  const { data: dentists = [], isLoading } = useGetAvailableDoctors();
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-semibold mt-5">Choose Your Dentist</h2>

      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {dentists.map((dentist) => (
          <Card
            key={dentist.id}
            className={`cursor-pointer transition-all hover:shadow-lg hover:border-primary/60 ${
